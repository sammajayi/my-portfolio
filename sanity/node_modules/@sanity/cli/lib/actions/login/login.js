...
async function login(args, context) {
  ...
  const {
    server,
    token: tokenPromise
  } = await startServerForTokenCallback({
    apiHost,
    apiClient
  });
  ...
  const platformName = _os.default.platform();
  const platform = platformName in platformNames ? platformNames[platformName] : platformName;
  const hostname = _os.default.hostname().replace(/\.(local|lan)$/g, '');
...
function startServerForTokenCallback(options) {
  ...
  return new Promise((resolve, reject) => {
    const server = _http.default.createServer(async function onCallbackServerRequest(req, res) {
